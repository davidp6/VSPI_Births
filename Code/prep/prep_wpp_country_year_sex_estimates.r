...
#
# 3/29/2017
# Prep WPP births estimates by country-year-sex
# ----------------------------------------------
...
# input file
inFile = './Data/Raw/WPP/WPP2015_FERT_F02_SEX_RATIO_AT_BIRTH.XLS'

# country codes
ccFile = './Data/Country_Data/countrycodes.csv'

# output file
outFile = './Data/Country_Data/WPP_Country_Year_Sex_Estimates.csv'
# --------------------------------------------------------------------
...
data = melt(data, id.vars=idVars, variable.name='year', value.name='mf_ratio')
data[, year:=str_sub(year, 1, 4)]
data[, year:=as.numeric(year)+2]

# expand to all years
blankData = data[year==min(year)]
blankData[, mf_ratio:=NA]
...
for(i in unique(data$iso3)) {
	fitData = data[iso3==i]
	lFit = smooth.spline(y=fitData[!is.na(mf_ratio)]$mf_ratio, fitData[!is.na(mf_ratio)]$year)
...
ggplot(data[iso3 %in% sample(unique(data$iso3), 6)], aes(y=mf_ratio, x=year)) + 
	geom_point() + 
